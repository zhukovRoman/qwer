...
class EAuth extends CApplicationComponent {
	...
	protected function getService($service) {
		...
		if (!isset($services[$service]))
			throw new EAuthException(Yii::t('eauth', 'Undefined service name: {service}.', array('{service}' => $service)));
	...
	public function getIdentity($service) {
		...
		if (!isset($this->services[$service]))
			throw new EAuthException(400, Yii::t('eauth', 'Undefined service name: {service}.', array('{service}' => $service)));
 ...
 * @version 1.0
 */
class EAuthException extends CHttpException {}