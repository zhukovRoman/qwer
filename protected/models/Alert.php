...
class Alert extends CActiveRecord {
    ...
    public static function showAlerts() {
        ...
        $res = '';
        foreach ($alerts as $a) {
            $res .= "<div class='alert span8 $a->type'>" .
                    '<button type="button" class="close" data-dismiss="alert">x</button>' .
                    $a->text;
            if (Yii::app()->user->checkAccess('pagesManage')) {
                $res .= CHtml::link("Удалить", Yii::app()->createUrl('alert/delete', array('id' => $a->id)));
                $res .= CHtml::link("Изменить", Yii::app()->createUrl('alert/update', array('id' => $a->id)));
            }
            $res .= '</div>';
        }
        if (Yii::app()->user->checkAccess('pagesManage')) { 
            $res .= CHtml::link("Добавить подсказку", Yii::app()->createUrl('alert/create'));
        }
        return $res;
    }
