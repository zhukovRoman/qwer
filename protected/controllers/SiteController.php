...
//            $resArray = Yii::App()->search->searchRaw($searchCriteria); // array result
    ...
    public function actionLogin($linkService = null) {
        ...
        if (!Yii::app()->user->isGuest)
            $this->redirect(Yii::app()->createUrl('account/view', array('id' => Yii::app()->user->getId()))); //(Yii::app()->baseUrl);

            
// 1. В случае входа для привязки аккаунта, выводим сообщение
        $linking = Yii::app()->request->getQuery('linkService');
        if (isset($linking))
            Yii::app()->user->setFlash('warning', 'Уважаемый пользователь! Для того, чтобы привязать аккаунт к уже существующему, войдите в него');
    ...
    public function actionUploadAvatar($parent_id) {
        Yii::import("ext.EAjaxUpload.qqFileUploader");

        $folder = "uploads/$parent_id/";
        ...
        $result = $uploader->handleUpload($folder, false, true, true);

        //$fileSize=filesize($folder.$result['filename']);//GETTING FILE SIZE
        //$fileName=$result['filename'];//GETTING FILE NAME
        $url = $result['filename']; // путь с расширением
        //print_r($result); echo $url; die();
        Yii::app()->ih
                ->load($url)
